      ...
      <nav className="relative z-10 border-b border-white/40 bg-white/70 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <Link href="/user/dashboard" className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-600 to-blue-500 text-white font-semibold shadow">
              CM
            </Link>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/user/courses" className="text-sm font-medium text-blue-600 transition">Courses</Link>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${activeTab === 'enrolled' ? 'bg-cyan-600 text-white' : 'bg-slate-100 text-slate-700'}`}
              >
                My Courses ({myEnrollments.length})
              </button>
              <button
                onClick={() => setActiveTab('browse')}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${activeTab === 'browse' ? 'bg-cyan-600 text-white' : 'bg-slate-100 text-slate-700'}`}
              >
                Browse All
              </button>
            </div>
            ...
            {loading ? (
              ...
              myEnrollments.length === 0 ? (
                <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-6 text-center">
                  ...
                  <button onClick={() => setActiveTab('browse')} className="mt-2 text-sm font-semibold text-cyan-600">
                    Browse courses
                  </button>
                </div>
              ) : (
                  ...
                  {myEnrollments.map((enrollment) => (
                    <div key={enrollment.id} className="rounded-2xl border border-white/60 bg-white/80 p-5 shadow-sm">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700">Enrolled</span>
                        {enrollment.course?.subject && (
                          <span className="rounded-full bg-cyan-100 px-2 py-0.5 text-xs text-cyan-700">{enrollment.course.subject.code}</span>
                        )}
                      </div>
                      <h4 className="text-lg font-semibold text-slate-900">{enrollment.course?.title}</h4>
                      <p className="mt-1 text-sm text-slate-500 line-clamp-2">{enrollment.course?.description || 'No description'}</p>
                  ...
                  {availableCourses.map((course) => {
                    ...
                    return (
                      <div key={course.id} className="rounded-2xl border border-white/60 bg-white/80 p-5 shadow-sm">
                        <div className="flex items-center gap-2 mb-2">
                          {isEnrolled && <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700">Enrolled</span>}
                          {course.subject && (
                            <span className="rounded-full bg-cyan-100 px-2 py-0.5 text-xs text-cyan-700">{course.subject.code}</span>
                          )}
                        </div>
